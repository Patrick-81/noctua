...
## En cours
- [ ] **Skymap — étoiles / constellations désalignées & miroir** (chantier demain) : les constellations se déplacent à l'opposé des étoiles et ne sont pas alignées sur leurs étoiles principales (ex: Orion). Cause probable : miroir X partiel (`sky-projection.js` flip `rawX` pour `projectStars` vs `sky-engine.js` wrap `w - x` pour D3 `constellations`) — double miroir ou `w` stale. À vérifier : `sky-projection.js:45,80` (`projectPoint`/`projectStars`) + `sky-engine.js:130,166,488` (`_rawProjection`/`_wrapMirrored`/`_mirrorGeoJson`/`render` + `_rawPathGenerator` pour MW). Test parité `tests/sky-projection.spec.js` vert mais visuel KO.
