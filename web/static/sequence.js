...
function seqTemplateSelected() {
    if (seqStatus.running || seqQuickCapture && seqQuickCapture.running) return;
    const sel = document.getElementById('seq-template-select');
    if (!sel || !sel.value) return;
    const t = _seqTemplates.find(x => x.name === sel.value);
    if (!t || !t.frames || !t.frames.length) return;
    // Template -> nouvelle cible Nina-like
    const target = seqNewTarget();
    target.name = t.name;
    target.steps = t.frames.map(f => ({
    ...
    seqData.targets.push(target);
    seqSelectedTargetId = target.id;
    seqRenderTargetList();
